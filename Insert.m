function [flag,insertScoreNew] = Insert(k,insertScore)
...
suminsertScore = sum(insertScore);
pinsertScore = insertScore ./ suminsertScore;
minsertScore = cumsum(pinsertScore);


rd = rand;
for i = 1:length(insertScore)
    if rd > minsertScore(i)
        continue;
    else
        beta = i;
        break;
...
switch beta
    case 1
        if initialDeteriorationNode(routeK(r,s1)) < initialDeteriorationNode(routeK(r,s2))
            temp = [routeK(r,1:s1-1) routeK(r,s2) routeK(r,s1:s2-1) routeK(r,s2+1:end)];
            routeK(r,:) = temp;
...
if fitK < fitness(k)
    insertScore(beta) = insertScore(beta) + abs(fitness(k) - fitK) ./ fitness(k);
    insertScoreNew = insertScore;
    fitness(k) = fitK;
    damage(k,:) = damageKnew;
    road(k,:) = roadKnew;
