...
nodeNum = 100;
depot = 101;                                                               % 仓库编号
data = load('rc101-25-para.txt');
x = data(:,2);                                                             % 受灾点的横纵坐标
y = data(:,3);
demand = data(:,4);                                                        % 受灾点的物资需求量
s = data(:,5);                                                             % 受灾点的服务时间
D(:,:,1) = load('rc101-25-Dshort.txt');
D(:,:,2) = load('rc101-25-D.txt');
D(:,:,3) = load('rc101-25-Dlong.txt');
nodeData = load('nodeRate.txt');
% clusterData = load('clusterNodeRate2.txt');
initialDeteriorationNode = nodeData(:,1);
deteriorationRateNode = nodeData(:,2);
% initialDeteriorationNode = clusterData(:,1);
% deteriorationRateNode = clusterData(:,2);
...
t = 1;
iterMax = 3000;
% carNum = floor(25*nodeNum/200);                                            % 单周期内可用的车辆数（多除了2）
carNum = 6; 
roadNum = 3;

totalCarNum = numPeriods * carNum;    % 总行数 = 周期数 * 每周期车数

% ==== 矩阵第一维从 carNum 改为 totalCarNum ====
route = zeros(totalCarNum,nodeNum,popsize);                                % 初始化route矩阵（修改过）
...
while t <= iterMax
    for k = 1:popsize
        ...
        while alpha <= 5
            switch alpha
                case 1
                    [flag,swapScoreNew] = Swap(k,swapScore);
                    swapScore = swapScoreNew;
                case 2
                    [flag,insertScoreNew] = Insert(k,insertScore);
                    insertScore = insertScoreNew;
                case 3
                    flag = Opt(k);
                case 4
                    [flag,exchangeScoreNew] = Exchange(k,exchangeScore);
                    exchangeScore = exchangeScoreNew;
                case 5
                    [flag,relocationScoreNew] = Relocate(k,relocationScore);
                    relocationScore = relocationScoreNew;
            end
...
% idx = load('omega3.txt');
% depotPoint = 0;
% for i = 1:globalBestNumR
%     tempRoute = [depot globalBestRoute(i,1:globalBestLengthR(i)) depot];
%     for j = 1:length(tempRoute)-1
%         plot([x(tempRoute(j)),x(tempRoute(j+1))],[y(tempRoute(j)),y(tempRoute(j+1))],'co-','MarkerEdgeColor','g','MarkerFaceColor','g');
%         if tempRoute(j+1) ~= depot
% %             text((x(tempRoute(j))+x(tempRoute(j+1)))/2,(y(tempRoute(j))+y(tempRoute(j+1)))/2,num2str(globalBestRoad(tempRoute(j+1))));
%         end
%         if tempRoute(j) ~= 101
% %             text(x(tempRoute(j))+1,y(tempRoute(j))+1,num2str(tempRoute(j)));
%         else
%             if depotPoint == 0
% %                 text(x(tempRoute(j))+1,y(tempRoute(j))+1,num2str(0));
%                 depotPoint = 1;
%             end
%         end
%         hold on
...
%     hold on
% end
% h1 = scatter(x(1),y(1),'MarkerEdgeColor','g','MarkerFaceColor','g');
% h2 = scatter(x(depot),y(depot),120,'rp','filled');
% % axis([10 50 45 90]);
...
% prob = zeros(1,10);
% for i = 1:carNum
%     prob((idx(globalBestRoute(i,1))-1)/eta+1) = prob((idx(globalBestRoute(i,1))-1)/eta+1) + 1;
% end
% probability = prob ./ sum(prob);
